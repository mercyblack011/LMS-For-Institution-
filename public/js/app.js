// ========== STATE ==========
const API = '/api';
let token = localStorage.getItem('vta_token') || null;
...
function saveSession(t, user) {
  ...
  localStorage.setItem('vta_token', t);
  localStorage.setItem('vta_user', JSON.stringify(user));
}
function clearSession() {
  token = null; currentUser = null;
  localStorage.removeItem('vta_token');
...
document.getElementById('registerBtn').addEventListener('click', async () => {
  ...
  const role = document.querySelector('#roleSelect .active')?.dataset.role || 'student';
  const errBox = document.getElementById('loginError');
  errBox.style.display = 'none';
  try {
...
document.getElementById('search').addEventListener('input', e => {
  ...
  document.querySelectorAll('.page.active .course').forEach(c => {
...
function courseIconHtml(course) {
  if (course.logo_url) return `<img src="${course.logo_url}" alt="${escapeHtml(course.name)}" style="width:100%;height:100%;object-fit:cover;border-radius:12px">`;
...
function openCourseModal(course = null) {
  ...
  if (course && course.logo_url) { logoPreview.src = course.logo_url; logoPreview.style.display = 'block'; } else { logoPreview.style.display = 'none'; }
...
async function renderStudents() {
  ...
  document.getElementById('studentList').innerHTML = students.map(s => `
    <div class="student-card">
      <div class="row">
        ${s.photo_url
          ? `<img class="avatar-lg" src="${s.photo_url}" alt="${escapeHtml(s.name)}">`
  ...
  document.querySelectorAll('#studentList [data-edit]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      if (s.photo_url) { preview.src = s.photo_url; preview.style.display = 'block'; } else { preview.style.display = 'none'; }
...
async function renderInstructorLecturerDirectory() {
  ...
  container.innerHTML = peers.map(l => `
    <div class="student-card">
      <div class="row">
        ${l.photo_url
          ? `<img class="avatar-lg" src="${l.photo_url}" alt="${escapeHtml(l.name)}">`
          : `<div class="avatar-lg">${l.name.split(' ').map(w => w[0]).join('').slice(0, 2)}</div>`}
        <div>
          <b>${escapeHtml(l.name)}</b><br>
...
document.getElementById('saveLecturerBtn').addEventListener('click', async () => {
...
function openLecturerEditModal(l) {
  ...
  if (l.photo_url) { preview.src = l.photo_url; preview.style.display = 'block'; } else { preview.style.display = 'none'; }
...
<fill><patternFill patternType="solid"><fgColor rgb="FFE7EDFF" /><bgColor indexed="64" /></patternFill></fill>
</fills>
<borders count="3">
...
document.getElementById('dyPrintGenerateBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    box.innerHTML = submissions.map(s => `
      <div class="submission-row">
        ...
        <div class="grade-row">
          <button class="btn btn-outline-dark btn-sm" data-preview-file="${s.file_path || ''}" data-preview-name="${escapeHtml(s.file_name || '')}" data-preview-title="${escapeHtml(s.student_name)}'s Submission"><svg class="icon sm"><use href="#i-eye"/></svg> Preview</button>
          <a class="btn btn-outline-dark btn-sm" href="${s.file_path || ''}" download="${escapeHtml(s.file_name || '')}"><svg class="icon sm"><use href="#i-download"/></svg> Download</a>
...
async function renderAssignments() {
  ...
  container.innerHTML = assignments.map(a => {
    ...
    if (currentUser.role === 'student') {
      ...
      if (a.file_path) actionParts.push(`<button class="btn btn-outline-dark btn-sm" data-preview-file="${a.file_path}" data-preview-name="${escapeHtml(a.file_name || '')}" data-preview-title="${escapeHtml(a.title)}"><svg class="icon sm"><use href="#i-eye"/></svg> Preview Attachment</button>`);
      if (a.my_submission) {
        actionParts.push(`<span class="badge green">Submitted${a.my_grade ? ' &middot; Grade: ' + escapeHtml(a.my_grade) : ''}</span>`);
        actionParts.push(`<button class="btn btn-outline-dark btn-sm" data-preview-file="${a.my_submission_file}" data-preview-name="${escapeHtml(a.my_submission_file_name || '')}" data-preview-title="My Submission"><svg class="icon sm"><use href="#i-eye"/></svg> View My Submission</button>`);
...
document.getElementById('saveExamDeadlineBtn').addEventListener('click', async () => {
...
document.getElementById('confirmExamSubmitBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    box.innerHTML = submissions.map(s => `
      <div class="submission-row">
        ...
        <div class="grade-row">
          <button class="btn btn-outline-dark btn-sm" data-preview-file="${s.file_path || ''}" data-preview-name="${escapeHtml(s.file_name || '')}" data-preview-title="${escapeHtml(s.student_name)}'s Exam Paper"><svg class="icon sm"><use href="#i-eye"/></svg> Preview</button>
          <a class="btn btn-outline-dark btn-sm" href="${s.file_path || ''}" download="${escapeHtml(s.file_name || '')}"><svg class="icon sm"><use href="#i-download"/></svg> Download</a>
          <input class="grade-input" placeholder="Grade" value="${escapeHtml(s.grade || '')}" data-grade-for="${s.id}">
          <input class="feedback-input" placeholder="Feedback" value="${escapeHtml(s.feedback || '')}" data-feedback-for="${s.id}">
          <button class="btn btn-gold btn-sm" data-save-grade="${s.id}"><svg class="icon sm"><use href="#i-check"/></svg> Save</button>
...
async function renderExams() {
  ...
  container.innerHTML = exams.map(a => {
    ...
    if (currentUser.role === 'student') {
      ...
      if (a.my_submission) {
        actionParts.push(`<span class="badge green">Submitted${a.my_grade ? ' &middot; Grade: ' + escapeHtml(a.my_grade) : ''}</span>`);
        actionParts.push(`<button class="btn btn-outline-dark btn-sm" data-preview-file="${a.my_submission_file}" data-preview-name="${escapeHtml(a.my_submission_file_name || '')}" data-preview-title="My Exam Paper"><svg class="icon sm"><use href="#i-eye"/></svg> View My Upload</button>`);
...
async function renderResourceList(type) {
  ...
  container.innerHTML = resources.map(r => `
    <div class="assignment-item">
      ...
      <div class="actions">
        <button class="btn btn-outline-dark btn-sm" data-preview-file="${r.file_path}" data-preview-name="${escapeHtml(r.file_name)}" data-preview-title="${escapeHtml(r.unit_name)}"><svg class="icon sm"><use href="#i-eye"/></svg> Preview</button>
        <a class="btn btn-outline-dark btn-sm" href="${r.file_path}" download="${escapeHtml(r.file_name)}"><svg class="icon sm"><use href="#i-download"/></svg> Download</a>
  ...
  container.innerHTML = events.map(ev => `
    <div class="event-card">
      ...
      ${ev.photos.length ? `<div class="event-photo-grid">${ev.photos.map(p => `
        <img class="event-thumb" src="${p.photo_path}" data-preview-file="${p.photo_path}" data-preview-title="${escapeHtml(ev.name)}">
      `).join('')}</div>` : ''}
      <div class="actions" style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap">
        ${ev.pdf_path ? `
          <button class="btn btn-outline-dark btn-sm" data-preview-file="${ev.pdf_path}" data-preview-name="${escapeHtml(ev.pdf_name || '')}" data-preview-title="${escapeHtml(ev.name)}"><svg class="icon sm"><use href="#i-eye"/></svg> View PDF</button>
          <a class="btn btn-outline-dark btn-sm" href="${ev.pdf_path}" download="${escapeHtml(ev.pdf_name || '')}"><svg class="icon sm"><use href="#i-download"/></svg> Download</a>
        ` : ''}
        ${isAdmin ? `
...
async function renderProfile() {
  ...
  if (photoUrl) { photoImg.src = photoUrl; photoImg.style.display = 'block'; photoInitials.style.display = 'none'; }
...
document.getElementById('pfChangePassBtn').addEventListener('click', async () => {
